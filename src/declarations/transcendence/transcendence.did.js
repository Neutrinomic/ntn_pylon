export const idlFactory = ({ IDL }) => {
  ...
  const ModifyRequest__8 = IDL.Record({ 'description' : IDL.Text });
  ...
  const ModifyRequest__4 = IDL.Record({ 'flow' : Flow, 'range' : Range });
  const ModifyRequest__2 = IDL.Record({
    'token_ratios' : IDL.Vec(IDL.Nat),
    'remove_amount_usd' : IDL.Nat,
    'remove_interval_seconds' : IDL.Nat64,
    'price_ledger_id' : IDL.Principal,
    'threshold_percent' : IDL.Float64,
    'rebalance_interval_seconds' : IDL.Nat64,
    'swap_amount_usd' : IDL.Nat,
  });
  const Mode = IDL.Variant({ 'remove' : IDL.Null, 'auto' : IDL.Null });
  const ModifyRequest__1 = IDL.Record({
    'mode' : Mode,
    'interval_seconds' : IDL.Nat64,
    'range_percent' : IDL.Float64,
    'remove_percent' : IDL.Float64,
  });
  ...
  const CreateRequest__8 = IDL.Record({
    'init' : IDL.Record({}),
  ...
  const Shared__2 = IDL.Record({
    'internals' : IDL.Record({
      'last_error' : IDL.Opt(IDL.Text),
      'last_remove' : IDL.Nat64,
      'current_ratios' : IDL.Vec(IDL.Float64),
